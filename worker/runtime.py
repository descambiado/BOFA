...
class WorkerRuntime:
        ...
        checks.update(
            {
                ...
                "image_digest": bool(
                    profile.image and profile.image_digest and profile.image_digest.startswith("sha256:")
                ),
    ...
    def execute(self, envelope: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        if expires_at.tzinfo is None:
            expires_at = expires_at.replace(tzinfo=timezone.utc)
        timeout_seconds = max(1, int((expires_at - datetime.now(timezone.utc)).total_seconds()))
        effective_limits = dict(envelope["manifest"].get("effective_limits") or {})
        max_output_bytes = max(1024, int(effective_limits.get("max_output_bytes") or 10 * 1024 * 1024))
        started_at = _utc_timestamp()
        try:
            if run_type == "script":
                result = self._execute_script(metadata, timeout_seconds)
            elif run_type == "flow":
    ...
    def _execute_script(self, metadata: Dict[str, Any], timeout_seconds: int) -> Dict[str, Any]:
        ...
        if self.script_executor:
            return dict(self.script_executor(module, script, parameters))
