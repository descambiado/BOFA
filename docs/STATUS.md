...
| **Modulos y scripts** | OK Operativo | 20 modulos, 67 scripts. Verificacion `tools/verify_bofa.py --full` -> 0 fallos. Paramentos `--key value`, YAML alineado. |
| **Flujos (BOFA Flow)** | OK Listo | `config/flows/` (demo, recon, blue, web_recon, pentest_basic, vulnerability_scan, full_recon, vuln_triage, web_security_review). `flows/flow_runner.py`: list_flows, run_flow, informes Markdown en `reports/`. Orquestables por LLM. |
...
| Scripts | 68 |
| Flujos predefinidos | 9 (demo, recon, blue, web_recon, pentest_basic, vulnerability_scan, full_recon, vuln_triage, web_security_review) |
